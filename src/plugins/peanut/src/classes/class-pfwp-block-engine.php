...
class PFWP_Block_Engine {
  ...
  private static $pre_render_filters;
    ...
    public static function initialize() {
      ...
      self::$blocks = PFWP_Assets_Engine::get_assets( 'blocks' );
      ...
      foreach(self::$blocks as $key => $value) {
        ...
        if ( str_starts_with($key, 'php_index' ) ) {
          $baseKey = PFWP_Assets_Engine::get_base_key( 'blocks', $key, 'php_index' );
          require_once( get_template_directory() . '/blocks/' . $baseKey . '/index.php' );
        }
      }

      if ( property_exists( $pfwp_global_config, 'core_block_filters' ) ) {
        self::$render_filters = property_exists( $pfwp_global_config->core_block_filters, 'on_render' ) ? $pfwp_global_config->core_block_filters->on_render : (object) [];
        self::$pre_render_filters = property_exists( $pfwp_global_config->core_block_filters, 'on_pre_render' ) ? $pfwp_global_config->core_block_filters->on_pre_render : (object) [];
      } else {
        self::$render_filters = (object) [];
        self::$pre_render_filters = (object) [];
      }
    }
    ...
    public static function register() {
      foreach(self::$blocks as $key => $value) {
        ...
        if ( str_starts_with($key, 'editor' ) ) {
          $baseKey = PFWP_Assets_Engine::get_base_key( 'blocks', $key, 'editor' );
          $assets = PFWP_Component_Engine::process_assets( PFWP_Assets_Engine::get_key_assets( 'blocks', $baseKey, 'editor' ) );
    ...
    public static function component_render_callback( $attributes, $content, $block ) {
        ...
        if ('' !== locate_template( 'components/' . $assetKey . '/index.php', false, false ) ) {
            return PFWP_Component_Engine::get_template_part( 'components/' . $assetKey . '/index', null, array( 'attributes' => $attributes, 'content' => $content ) );
        } else {
            return $content;
        }
    }
    ...
    public static function filter_render( $block_content, $block ) {
      ...
      if ( property_exists( self::$render_filters, $blockName ) && '' !== locate_template( self::$render_filters->$blockName . '/index.php' ) ) {
          return PFWP_Component_Engine::get_template_part( self::$render_filters->$blockName . '/index', null, array( 'filter_type' => 'render', 'attributes' => $block['attrs'], 'content' => $block_content ) );
      } else if ( property_exists( self::$render_filters, $blockName ) ) {
          // If no PHP exists for filter, still add component CSS and JS assets if they exists
          PFWP_Component_Engine::process_template_part( self::$render_filters->$blockName . '/index' );
      }
    ...
    public static function filter_pre_render( $pre_render, $parsed_block, $parent_block ) {
      ...
      if ( property_exists( self::$pre_render_filters, $blockName ) && '' !== locate_template( self::$pre_render_filters->$blockName . '/index.php' ) ) {
          return PFWP_Component_Engine::get_template_part( self::$pre_render_filters->$blockName . '/index', null, array( 'filter_type' => 'pre_render', 'attributes' => $block['attrs'], 'inner_blocks' => $block['innerBlocks'] ) );
      } else if ( property_exists( self::$pre_render_filters, $blockName ) ) {
          // If no PHP exists for filter, still add component CSS and JS assets if they exists
          PFWP_Component_Engine::process_template_part( self::$pre_render_filters->$blockName . '/index' );
      }
...
add_filter( 'pre_render_block', array( 'PFWP_Block_Engine', 'filter_pre_render' ), 99999, 3 );

// add_filter( 'render_block_data', array( 'PFWP_Block_Engine', 'filter_data' ), 99999, 3 );

add_filter( 'render_block', array( 'PFWP_Block_Engine', 'filter_render' ), 99999, 2 );

add_action( 'after_setup_theme', array( 'PFWP_Block_Engine', 'initialize' ), 2 );

add_action( 'init', array( 'PFWP_Block_Engine', 'register' ), 2 );
