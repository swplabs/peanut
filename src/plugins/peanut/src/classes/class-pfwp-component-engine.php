...
class PFWP_Component_Engine {
    ...
    public static function process_assets( $component_assets ) {
      ...
      if ( property_exists( $component_assets, 'css' ) ) {
        foreach ($component_assets->css as $key => $value) {
          // array_push( $assets->css, '<style>' . PFWP_Assets_Engine::get_content( $value ) . '</style>' . PHP_EOL );
    ...
    public static function process_template_part( $slug, $name = null ) {
      ...
      if ( $key && !property_exists( self::$components, $key ) ) {
          ...
          self::$js_data->$key = (object) [];

          if ( PFWP_Assets_Engine::has_asset( 'components', $key, 'styles' ) ) {
            $style_assets = self::process_assets( PFWP_Assets_Engine::get_key_assets( 'components', $key, 'styles' ) );
          ...
          if ( PFWP_Assets_Engine::has_asset( 'components', $key, '' ) ) {
            $client_assets = self::process_assets( PFWP_Assets_Engine::get_key_assets( 'components', $key, '' ) );
      ...
      foreach ( self::$components as $key => $value ) {
        if ( property_exists( $value->assets, 'js' ) ) {
          foreach ( $value->assets->js as $asset_key => $asset_value ) {
            echo '<script src="' . $asset_value . '"></script>' . PHP_EOL;
          }
        }
      }
      ...
      foreach ( self::$components as $key => $value ) {
        if ( property_exists( $value->assets, 'css' ) ) {
          foreach ( $value->assets->css as $asset_key => $asset_value ) {
            $styles .= '<link rel="stylesheet" href="' . $asset_value . '" />' . PHP_EOL;
          }
        }
      }
...
add_action('after_setup_theme', array( 'PFWP_Component_Engine', 'initialize' ), 2 );

add_action( 'get_template_part', array( 'PFWP_Component_Engine', 'process_template_part' ), 10, 3 );

add_action( 'template_redirect', array( 'PFWP_Component_Engine', 'capture_ob' ) );

add_action( 'wp_footer', array( 'PFWP_Component_Engine', 'inline_instance_js_data' ), 998 );

add_action( 'wp_footer', array( 'PFWP_Component_Engine', 'inject_footer' ), 1000 );

add_action( 'wp_footer', array( 'PFWP_Component_Engine', 'process_ob' ), 2000 );

add_action( 'wp_head', array( 'PFWP_Component_Engine', 'mark_head' ), 1000 );
